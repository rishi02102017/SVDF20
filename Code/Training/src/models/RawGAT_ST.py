...
class GraphAttentionLayer(nn.Module):
    def __init__(self, in_dim, out_dim, **kwargs):
        ...
        # activate - use ReLU instead of SELU for stability
    ...
    def _init_new_params(self, *size):
        ...
        nn.init.kaiming_normal_(out, mode='fan_out', nonlinearity='relu')  # Better for ReLU
        return out
...
class Residual_block(nn.Module):
    def __init__(self, nb_filts, first = False):
        ...
        if not self.first:
			...
			padding = (1,1),
			stride = 1)
        self.selu = nn.ReLU(inplace=True)  # Changed from SELU to ReLU for stability
...
class Model(nn.Module):
    def __init__(self,  device):
        ...
        self.device=device
        # Use the training config directory instead of model directory
        dirname = '/data-caffe/rishabh/SingFake_Project/code_backup/training/model_configs'
        filename = 'rawgat_st_config.yaml' 
        cp_path = os.path.join(dirname,filename)
        with open(cp_path, 'r') as f_yaml:
